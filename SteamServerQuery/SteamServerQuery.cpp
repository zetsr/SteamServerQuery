...
ServerInfo query_server_info(const std::string& ip, int port) {
    ...
    info.ping = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ...
    if (recv_buf[4] == 0x49) { // Detailed info response
        ...
        // 解析服务器类型
        char server_type_char = recv_buf[offset++];
        switch (server_type_char) {
        case 'd':
...
int main() {
    SetConsoleTitle(L"github.com/zetsr/SteamServerQuery");
